...
function hw_3_2
    ...
    [T1,Y1] = heun(f, a, b, ya, 0.01);
    [T2,Y2] = heun(f, a, b, ya, 0.001);
    [T3,Y3] = heun(f, a, b, ya, 0.0001);
    
    figure(); plot(T1,Y1,T2,Y2,T3,Y3,T3,f_ex(T3))
    title("Heun Approx"); legend("h = 0.01","h = 0.001","h = 0.0001","Exact")
    
    relerr1 = relerr(f_ex, T1, Y1);
    relerr2 = relerr(f_ex, T2, Y2);
    relerr3 = relerr(f_ex, T3, Y3);
    figure(); plot(T1, relerr1, ':', T2, relerr2, '--', T3, relerr3, '-.')
    title("Heun Relative Error"); legend("h = 0.01","h = 0.001","h = 0.0001")
